    ...
    <div class="box box-solid">
        ...
        <div class="box-body">
            ...
            <table class="table table-bordered table-condensed table-striped table-hover" id="table-kategori" >
                ...
                <tbody>
                    @foreach($data as $dt)
                    <tr data-rowid="{{$rownum}}" data-kategoriid="{{$dt->id}}">
                        <td>{{$rownum++}}</td>
                        <td>{{$dt->nama}}</td>
                        <td>{{$dt->satuan}}</td>
                        <td>
                            <a class="btn btn-success btn-xs btn-edit-kategori" href="inventory/kategori/edit/{{$dt->id}}" ><i class="fa fa-edit" ></i></a>
                            @if($dt->can_delete == 1)
                            <a class="btn btn-danger btn-xs btn-delete-kategori" href="inventory/kategori/delete/{{$dt->id}}" ><i class="fa fa-trash" ></i></a>
                            @endif
                        </td>
                    </tr>
                    @endforeach
